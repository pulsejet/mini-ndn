...
ndn.fields.bin = ProtoField.bytes("ndn.bin", "Binary Data")
ndn.fields.len = ProtoField.uint32("ndn.len", "NDN Packet Length")
ndn.fields.type = ProtoField.string("ndn.type", "NDN Packet Type")
...
function ndn.dissector(tvb, pInfo, root) -- Tvb, Pinfo, TreeItem
   ...
   while (block.size <= nBytesLeft) do
      ...
      block.tree:add(ndn.fields.bin, tvb(0,tvb:len()))
      block.tree:add(ndn.fields.len, tvb:len())
      block.tree:add(ndn.fields.type, pktType)
